...
import { Upload, Wand2, Download, Loader2, X, AlertCircle, Video } from 'lucide-react'
...
function ThumbnailStudioContent() {
  ...
  const processFiles = async (fileList: File[] | FileList) => {
    ...
    try {
      ...
      // Deduplicate against already added images
      ...
      if (!uniqueIncoming.length) {
        setUploadError('No new images to add.')
        return
      }
      ...
      setUploadError(e instanceof Error ? e.message : 'Failed to process images')
    } finally {
    ...
    if (isProcessing && generationStartTime) {
      interval = setInterval(() => {
        ...
        if (estimatedDuration > 0) {
          ...
          setProgress(pct)
          const rem = Math.max(0, Math.ceil(estimatedDuration - elapsed))
  ...
  const handleGenerateThumbnail = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const j = await res.json()
        if (typeof j.credits === 'number') setCredits(j.credits)
        if ((j.credits ?? 0) < totalCost) {
          alert(`Insufficient credits! Need ${totalCost}.`)
          return
        }
      }
    } catch {}
    ...
    try {
      ...
      if (mainTitle.trim()) {
        formData.append('mainTitle', mainTitle.trim())
      }
      formData.append('mode', 'thumbnail-studio')
      
      const resp = await fetch('/api/thumbnail-studio', { method: 'POST', body: formData })
      if (!resp.ok) {
        const err = await resp.json().catch(() => ({}))
        throw new Error(err.error || `Failed: ${resp.status}`)
      }
      const json = await resp.json()
      setEditedImage(json.thumbnailUrl)
      ...
      alert(e instanceof Error ? e.message : 'Thumbnail generation failed')
    } finally {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="text-center flex-1">
            <h1 className="text-4xl font-bold text-foreground mb-2 text-balance">Thumbnail Studio</h1>
        ...
        <div className="space-y-8">
          <CreditDisplay onPurchaseCredits={async () => {
            setIsProcessingPayment(true)
            try {
              const res = await fetch('/api/checkout', { method: 'POST' })
              const data = await res.json().catch(() => null as any)
              if (!res.ok) throw new Error((data as any)?.error || (data as any)?.code || `HTTP ${res.status}`)
              if (data?.url) {
            ...
            <div className="space-y-6">
              ...
              <Card className="border-border hover:border-primary/50 transition-colors">
                <CardContent className="p-5 space-y-3">
                  <h4 className="text-md font-semibold text-foreground">Main Title (Optional)</h4>
                  <input
                    type="text"
                    value={mainTitle}
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Optional: Add a main title for your thumbnail</p>
                </CardContent>
              </Card>

              <Card className="border-border hover:border-primary/50 transition-colors">
                <CardContent className="p-5 space-y-3">
                  <h4 className="text-md font-semibold text-foreground">Thumbnail Description</h4>
                  <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder="e.g. Create a bold, eye-catching YouTube thumbnail with dramatic lighting, bright colors, and text overlay. Make it stand out in search results."
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Describe the style and elements you want in your thumbnail</p>
                </CardContent>
              </Card>
                  ...
                  <CardContent className="p-6 space-y-4">
                    <div className="space-y-3">
                      <Button
                        ...
                        disabled={isProcessing || credits < CREDIT_COST_PER_EDIT || !prompt.trim() || images.length === 0}
                        className="w-full"
                        size="lg"
                      >
                        {isProcessing ? (
                          ...
                          <>
                            <Video className="mr-2 h-4 w-4" />Generate Thumbnail ({CREDIT_COST_PER_EDIT} credit)
                          </>
                        )}
                      </Button>
                    </div>
            ...
            <div className="space-y-6 sticky top-8 self-start">
              <Card className="min-h-[400px] flex flex-col">
                <CardContent className="p-6 flex-1 flex flex-col">
                  <h3 className="text-lg font-semibold text-foreground mb-4 text-center">Thumbnail Preview</h3>
                  <div className="relative w-full aspect-video rounded-lg overflow-hidden bg-muted flex items-center justify-center">
                    {editedImage ? (
                      <Image src={editedImage} alt="Generated thumbnail" fill className="object-cover" />
                    ) : isProcessing ? (
                      ...
                      <div className="text-sm text-muted-foreground text-center px-4">
                        {images.length > 0
                          ? 'Add a description and click Generate Thumbnail.'
                  ...
                  {editedImage && (
                    <div className="mt-4 flex justify-center">
                      <Button onClick={downloadThumbnail} variant="outline" size="sm">
