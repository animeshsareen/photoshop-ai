...
import { Upload, Download, Loader2, X, AlertCircle, Eraser } from "lucide-react"
...
function TransparentContent() {
  ...
  const handleGenerateTransparent = async () => {
    ...
    try {
      ...
      const resp = await fetch("/api/transparent", {
        method: "POST",
        body: formData,
      })
      if (!resp.ok) {
      ...
      setTransparentUrl(json.transparentUrl || json.imageUrl || json.url || null)
      try {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="text-center flex-1">
            <h1 className="text-4xl font-bold text-foreground mb-2 text-balance">Transparent</h1>
            <p className="text-muted-foreground text-lg text-pretty">Welcome back, {user?.name}</p>
            <p className="text-muted-foreground text-lg text-pretty">
              Remove the background from product shots, portraits, or logos while keeping a crisp transparent PNG.
            </p>
          </div>
          <UserProfile />
        </div>
                  ...
                  >
                    ...
                    {!image ? (
                      <>
                        ...
                        <p className="text-sm text-muted-foreground mb-2">Upload an image to make the background transparent.</p>
                ...
                <CardContent className="p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">Cost per background removal</p>
                  ...
                  <p className="text-xs text-muted-foreground text-center">
                    {image ? "Ready when you are. Remove the background in seconds." : "Upload an image to get started."}
                  </p>
                </CardContent>
              </Card>
              ...
              <Card className="border-border">
                <CardContent className="p-5 space-y-3 text-sm text-muted-foreground">
                  ...
                  <ul className="space-y-2">
                    <li>• Use sharp images with good contrast between subject and background.</li>
                    <li>• Large product shots and portraits work especially well.</li>
                    <li>• Download delivers a transparent PNG ready for design tools.</li>
                  </ul>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="space-y-6">
              <Card className="border-border">
                <CardContent className="p-5 space-y-4">
                  ...
                  {image ? (
                    transparentUrl ? (
                      <BeforeAfterSlider
                        ...
                        afterSrc={transparentUrl}
                        beforeAlt="Original image"
                        afterAlt="Background removed image"
                  ...
                  {transparentUrl ? (
                    <Button variant="outline" onClick={downloadTransparent} className="w-full">
                      <Download className="mr-2 h-4 w-4" />
                      Download PNG
                    </Button>
                  ) : (
                    <p className="text-xs text-muted-foreground text-center">
                      {image ? "Remove the background to compare results." : "Upload an image to get started."}
                    </p>
                  )}
                </CardContent>
              </Card>
