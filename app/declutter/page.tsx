...
import { Upload, Wand2, Download, Loader2, X, AlertCircle } from 'lucide-react'
...
function DeClutterContent() {
  ...
  const removeImage = () => { setImage(null); setEditedImage(null); setSubsectionValue(null) }
  ...
  try { const res = await fetch('/api/credits', { cache: 'no-store' }); if (res.ok) { const j = await res.json(); if (typeof j.credits === 'number') setCredits(j.credits); if ((j.credits ?? 0) < CREDIT_COST_PER_EDIT) { alert(`Insufficient credits! Need ${CREDIT_COST_PER_EDIT}.`); return } } } catch {}
    ...
    try {
      ...
      // DeClutter mode flag
      formData.append('mode', 'declutter')
  ...
  window.dispatchEvent(new Event('creditsUpdated'))
    ...
    } catch (e) { alert(e instanceof Error ? e.message : 'DeClutter failed') }
    finally { setIsProcessing(false) }
  }

  const downloadEditedImage = () => { if (!editedImage) return; const a = document.createElement('a'); a.href = editedImage; a.download = 'decluttered-image.png'; document.body.appendChild(a); a.click(); document.body.removeChild(a) }
              ...
              <Card className="border-border hover:border-primary/50 transition-colors">
                <CardContent className="p-5 space-y-3">
                  ...
                  <textarea
                    ...
                    placeholder="e.g. keep the subject, remove floor clutter and wall posters"
                  ...
                  />
                  <p className="text-xs text-muted-foreground">Optional: Select a sub-section to restrict cleanup to that area.</p>
                  ...
                  <CardContent className="p-6 space-y-4">
                    <div className="space-y-3">
                      ...
                      >
                        {isProcessing ? (
                          <>
                            <Loader2 className="mr-2 h-4 w-4 animate-spin" />Cleaning image...
                          </>
                        ) : (
                          <>
                            <Wand2 className="mr-2 h-4 w-4" />DeClutter ({CREDIT_COST_PER_EDIT} credit)
                          </>
                        )}
                      </Button>
                    </div>
              ...
              <Card className="min-h-[400px] flex flex-col">
                <CardContent className="p-6 flex-1 flex flex-col">
                  ...
                  <div className="relative w-full aspect-square rounded-lg overflow-hidden bg-muted flex items-center justify-center">
                    {editedImage ? (
                      <Image src={editedImage} alt="Decluttered image" fill className="object-cover" />
                    ) : isProcessing ? (
                      <div className="flex flex-col items-center gap-3 w-full px-8">
                        <Loader2 className="h-8 w-8 animate-spin text-muted-foreground" />
                        <Progress value={progress} className="w-full h-2" />
                        <p className="text-xs text-muted-foreground">
                          Cleaning... {progress.toFixed(0)}%
                      ...
                      <div className="text-sm text-muted-foreground text-center px-4">
                        {image ? 'Click DeClutter to clean background noise.' : 'Upload an image to begin.'}
                      </div>
                    )}
                  </div>
                  {editedImage && (
