...
async function getOrCreateDevice(supabase: ReturnType<typeof getSupabaseAdmin>, deviceId: string, ip: string | null) {
  // Try reading the device row; if the table doesn't exist, the supabase client will return an error
  ...
  if (!data) {
    ...
    return inserted as { device_id: string; ip_address: string | null; credits: number }
  }
  if (ip && ip !== data.ip_address) {
    await supabase.from("device_credits").update({ ip_address: ip }).eq("device_id", deviceId)
  }
  return data as { device_id: string; ip_address: string | null; credits: number }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    // If authenticated, prefer user-based credits
    if (userEmail) {
    ...
    // Fallback to device-based tracking for unauthenticated requests
    const { key, mode, deviceId } = resolveTrackingKey(req, h)
    try {
      const row = await getOrCreateDevice(supabase, key, ip)
      return NextResponse.json({ key, mode, deviceId, ip, credits: row.credits })
    } catch (err: any) {
      const msg = err?.message || String(err)
      if (msg.includes("does not exist") || msg.includes("relation") || msg.includes("device_credits")) {
        console.error("[credits] Device tables missing in Supabase:", msg)
        return NextResponse.json({ error: "Missing device tables in Supabase. Create `device_credits` and `credit_ledger` or use authenticated users." }, { status: 500 })
      }
      throw err
    }
  } catch (e: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    // If authenticated -> user ledger flow
    if (userEmail) {
    ...
    // Unauthenticated -> device-based flow
    const { key, mode, deviceId } = resolveTrackingKey(req, h)
    try {
      ...
      const msg = err?.message || String(err)
      if (msg.includes("does not exist") || msg.includes("relation") || msg.includes("device_credits") || msg.includes("credit_ledger")) {
        console.error("[credits] Device tables missing in Supabase:", msg)
        return NextResponse.json({ error: "Missing device tables in Supabase. Create `device_credits` and `credit_ledger` or use authenticated users." }, { status: 500 })
      }
      throw err
