...
export async function GET(req: NextRequest) {
  try {
    ...
    // Require authentication for credit queries
    if (!userEmail) {
      return NextResponse.json({ error: "Authentication required to access credits" }, { status: 401 })
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Require authentication for credit updates
    if (!userEmail) {
      return NextResponse.json({ error: "Authentication required to modify credits" }, { status: 401 })
    }
