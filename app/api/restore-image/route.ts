...
Output:
  ...
  • Enhance visual quality while maintaining historical accuracy`
    ...
    try {
      ...
      console.log("[restore-image] Full response result:", JSON.stringify(result, null, 2))
      
      if (result && result.candidates && result.candidates[0]) {
        const candidate = result.candidates[0]
        console.log("[restore-image] First candidate:", {
        ...
        if (candidate.content && candidate.content.parts && candidate.content.parts[0]) {
          ...
          console.log("[restore-image] First part:", {
          ...
          if (firstPart.inlineData) {
            ...
            console.log("[restore-image] Successfully extracted image data, length:", generatedImageData?.length)
          } else {
            console.error("[restore-image] No inlineData found in first part")
          }
        } else {
          console.error("[restore-image] No content or parts found in first candidate")
        }
      } else {
        console.error("[restore-image] No candidates found in response")
      }

      if (!generatedImageData) {
        console.error("[restore-image] No image data extracted from Gemini response")
        console.error("[restore-image] Full response for debugging:", JSON.stringify(response, null, 2))
