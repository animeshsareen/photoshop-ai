...
import { Upload, Sparkles, Download, Loader2, X, AlertCircle } from "lucide-react"
...
const STYLE_OPTIONS = [
  { value: "corporate studio lighting, sharp focus, neutral background", label: "Corporate Studio" },
  { value: "natural window light, shallow depth of field, soft blur background", label: "Natural Light" },
  { value: "outdoor city backdrop, professional attire, golden hour lighting", label: "City Outdoor" },
]

type StyleOptionValue = typeof STYLE_OPTIONS[number]["value"]

function HeadshottedContent() {
  const { user } = useAuth()
  const [image, setImage] = useState<SelectedImage | null>(null)
  const [headshotUrl, setHeadshotUrl] = useState<string | null>(null)
  const [gender, setGender] = useState<string>("auto")
  const [aspectRatio, setAspectRatio] = useState<string>("1:1")
  const [stylePrompt, setStylePrompt] = useState<StyleOptionValue>(STYLE_OPTIONS[0]?.value ?? "")
  ...
  const estimateGenerationDuration = (bytes: number) => {
    ...
    return Math.min(40, Math.max(5, est))
  }
  ...
  const handleGenerateHeadshot = async () => {
    ...
    try {
      ...
      if (gender && gender !== "auto") formData.append("gender", gender)
      if (aspectRatio) formData.append("aspect_ratio", aspectRatio)
      if (stylePrompt) formData.append("style_prompt", stylePrompt)

      const resp = await fetch("/api/headshotted", {
        method: "POST",
        body: formData,
      })
      if (!resp.ok) {
      ...
      setHeadshotUrl(json.headshotUrl || json.imageUrl || json.url || null)
      try {
      ...
      alert(error instanceof Error ? error.message : "Headshot generation failed")
    } finally {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div className="text-center flex-1">
            <h1 className="text-4xl font-bold text-foreground mb-2 text-balance">Headshotted</h1>
            <p className="text-muted-foreground text-lg text-pretty">Welcome back, {user?.name}</p>
            <p className="text-muted-foreground text-lg text-pretty">
              Upload a selfie or portrait and generate a polished, professional headshot in seconds.
            </p>
          </div>
          <UserProfile />
        </div>
            ...
            <div className="space-y-6">
              ...
              <Card className="border-border hover:border-primary/50 transition-colors">
                <CardContent className="p-5">
                  ...
                  >
                    <h4 className="text-md font-semibold text-foreground mb-3">Source Photo</h4>
                    {!image ? (
                      <>
                        <Upload className="mx-auto h-10 w-10 text-muted-foreground mb-3" />
                        <p className="text-sm text-muted-foreground mb-2">Upload 1 clear portrait.</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                          <p>Max size: {formatFileSize(MAX_IMAGE_SIZE)}</p>
                          <p>Face should be well-lit and centered.</p>
                        </div>
                        <div className="mt-3">
              ...
              <Card className="border-border hover:border-primary/50 transition-colors">
                <CardContent className="p-5 space-y-4">
                  <div>
                    <h4 className="text-md font-semibold text-foreground mb-3">Headshot Settings</h4>
                    <div className="space-y-3">
                      <div>
                        <Label className="text-sm mb-2 block">Gender expression</Label>
                        <RadioGroup
                          value={gender}
                      ...
                      <div>
                        <Label className="text-sm mb-2 block">Lighting & vibe</Label>
                        <RadioGroup
                          value={stylePrompt}
                          onValueChange={(value) => setStylePrompt(value as StyleOptionValue)}
                        ...
                        >
                          {STYLE_OPTIONS.map((option) => (
                            <Label
                              ...
                              className={`border rounded-md py-2 px-3 text-sm font-medium cursor-pointer transition-colors text-left ${
                                stylePrompt === option.value
                                  ? "border-primary bg-primary/10 text-primary"
                                  : "border-border text-foreground hover:border-primary/50"
                              }`}
                            >
                  ...
                  <div className="rounded-md bg-muted/60 px-3 py-2 text-xs text-muted-foreground">
                    Tip: Use a high-resolution photo with a neutral expression for the cleanest result. Busy backgrounds will be
                ...
                <CardContent className="p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <p className="text-xs text-muted-foreground">Cost per headshot</p>
                  ...
                  <p className="text-xs text-muted-foreground text-center">
                    {image ? "Ready when you are. Click generate to transform your photo." : "Upload a source photo to get started."}
                  </p>
                </CardContent>
              </Card>
            </div>
            ...
            <div className="space-y-6">
              <Card className="border-border">
                <CardContent className="p-5 space-y-4">
                  ...
                  <div className="grid gap-4">
                    <div>
                      ...
                      <div className="relative aspect-square rounded-lg border border-dashed border-border bg-muted overflow-hidden">
                        {image ? (
                          <Image src={image.data} alt="Original" fill className="object-cover" sizes="320px" unoptimized />
                        ) : (
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-wide text-muted-foreground mb-2">Headshotted</p>
                      <div className="relative aspect-square rounded-lg border border-border bg-muted overflow-hidden">
                        {headshotUrl ? (
                          <Image src={headshotUrl} alt="Generated headshot" fill className="object-cover" sizes="320px" unoptimized />
                        ) : (
                          <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-sm">
                            <Sparkles className="h-8 w-8 mb-2" />
                            Headshot will appear here
                          </div>
                        )}
                      </div>
                    </div>
                  ...
                  {headshotUrl && (
                    <Button variant="outline" onClick={downloadHeadshot} className="w-full">
              ...
              <Card className="border-border">
                <CardContent className="p-5 space-y-3 text-sm text-muted-foreground">
                  <h4 className="text-md font-semibold text-foreground">Best results checklist</h4>
                  <ul className="space-y-2">
                    <li>• Use a recent photo where your face is unobstructed.</li>
                    <li>• Avoid hats, sunglasses, or extreme angles.</li>
                    <li>• Balanced, even lighting works best.</li>
                    <li>• Choose a vibe that matches your brand or role.</li>
                  </ul>
                </CardContent>
              </Card>
            </div>
...
export default function HeadshottedPage() {
  return (
    <ProtectedRoute>
      <HeadshottedContent />
    </ProtectedRoute>
